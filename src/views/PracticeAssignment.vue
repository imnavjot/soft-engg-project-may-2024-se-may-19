  ...
  <div class="container-fluid mt-5">
    <div class="row">
      ...
      <main class="col-md-9 ml-sm-auto col-lg-9 px-md-4" style="position: relative; width: 80%; margin-left: auto;">
        <h2 class="impressive-header">Practice Assignment</h2>
        ...
        <div v-else>
          <div v-for="(question, index) in questions" :key="question.id" class="mb-4">
            ...
            <button class="btn hint-button" @click="getHint(question.id)">Hint</button>
            <p v-if="hints[question.id]" class="result">{{ hints[question.id] }}</p>
          ...
          <button class="btn btn-primary mt-3 mb-3" @click="submitAnswers">Submit</button>
        </div>
...
export default {
  ...
  data() {
    return {
      ...
      hints: {}, // New property for storing hints
      loading: false
    };
  },
  created() {
  ...
  methods: {
    ...
    async fetchQuestions() {
      if (this.course && this.activeWeek) {
        ...
        this.answers = {};
        this.hints = {}; // Reset hints when fetching new questions
      }
    },
    toggleWeek(weekId) {
    ...
    async getHint(questionId) {
      try {
        ...
        const data = await response.json();
        if (response.ok) {
          console.log('Received hint:', data.hint);  // Debugging line
          this.hints[questionId] = data.hint;  // Direct assignment
        } else {
